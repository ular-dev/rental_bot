...
bot.onText(/\/filter/, (msg) => {
  ...
  bot.sendMessage(chatId, `Ваши фильтры:\n📍 Город: ${user.city || 'Не выбран'}\n🏙 Район: ${user.district || 'Не выбран'}\n🛏 Комнаты: ${user.roomFilter || 'Не выбрано'}\n\nЧто изменить?`, {
    reply_markup: {
      inline_keyboard: [
        [{ text: 'Город', callback_data: 'change_city' }, { text: 'Район', callback_data: 'change_district' }],
...
bot.on('callback_query', async (query) => {
  ...
  if (data === 'change_district') return sendDistrictSelection(chatId, users[chatId].city);

  if (data.startsWith('city_')) {
    const selectedCity = data.replace('city_', '');
    users[chatId].city = selectedCity;
    users[chatId].cityId = cities.find(c => c.name === selectedCity)?.id || null;
    saveUsers(users);
  ...
  if (data.startsWith('district_')) {
    ...
    const districtData = Object.values(districts).flat().find(d => d.name === selectedDistrict);
  ...
  if (data.startsWith('room_')) {
    ...
    saveUsers(users);
    await bot.sendMessage(chatId, `✅ Отлично! Вы выбрали: ${selectedRoom}
📬 Мы будем присылать вам актуальные квартиры каждые 20 минут.
🛑 Чтобы остановить рассылку, нажмите "Стоп" в меню /filter.`)
  ...
  if (data === 'stop_sending') {
    ...
    saveUsers(users);
    return bot.sendMessage(chatId, "🚫 Рассылка остановлена. Чтобы снова получать объявления, используйте /start");
  }
});
...
function sendDistrictSelection(chatId, city) {
  ...
  for (let i = 0; i < list.length; i += 2) {
    ...
    if (list[i + 1]) {
      row.push({ text: list[i + 1].name, callback_data: `district_${list[i + 1].name}` });
    }
    keyboard.push(row);
  }
...
bot.onText(/\/номер (\d+)/, (msg, match) => {
  ...
  for (const chatId in users) {
    const item = (users[chatId].sentItems || []).find(i => i.counter === requestedCounter);
    if (item) {
